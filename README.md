...
- Distance is tracked in real meters and speed in real m/s, with **no speed cap** — nothing ever silently absorbs your thrust. Whenever your current thrust (the Engine alone — this doesn't depend on tapping) can't hold you up, a scripted three-phase cooldown kicks in instead of just applying real gravity (which is far too weak to matter at relativistic speeds): a 2-second grace period where nothing changes, then a **linear** decay to zero at 10% of whatever your speed was *when cooling started* per second (so it always takes exactly 10 seconds to reach zero, however fast that was), then a guaranteed 10-second scripted fall down to the ground. Alternating `A`/`D` presses apply a thrust impulse (60 m/s — since stamina now only allows about one tap per ~2-second cycle without an engine running, that one tap needs to actually matter) and cost stamina; only genuine alternation (not repeats) counts, and consecutive alternations build a combo that increases thrust.
- **Stamina** is a backup for when the Engine isn't actively burning — while it's lit, tapping costs nothing; the moment it isn't (not yet ignited, or run dry), every tap drains stamina instead, and it only recharges once you're back on the ground (altitude 0), not passively over time. The base pool is tiny (0.1 at level 0) — a single unassisted tap fully exhausts it, so manual-only flight is basically a one-frame twitch until you invest in the **Stamina** upgrade (exponential per level) or get an Engine running. Run it out and you're "exhausted" — no more manual thrust, and (unless a strong-enough engine is covering you) gravity hits brutally harder — until it recovers to 25% (after landing, roughly a 2 second wait regardless of level, since the pool and the regen rate scale together).
...
- You start on solid green ground (buildings included), which gives way to visible Earth curvature starting around the stratosphere, then a full shrinking Earth sphere — oceans, continents, cloud swirls — as you keep climbing, the way it actually would.
- The Moon, the real planets, and every procedural solar system's star and planets are all sized up to feel suitably massive as you approach them.
...
- **Rebirth**: once you've crossed the whole Milky Way, a REBIRTH option unlocks in the shop. It's a real reset — your 67s and all four upgrade levels wipe back to 0 — in exchange for permanently **doubling your thrust**, stacking every time (×2, ×4, ×8, …). Achievements, best distance/velocity and systems-passed are lifetime records and are *not* reset. It's how you climb the same ladder again, faster each time.
- **Admin panel**: the shop's ADMIN tab is always visible but gated by a username/password login (single hardcoded credential pair — not a general account system). Once logged in (persists across sessions until you log out): MAX/buy-all buttons for each of the four upgrades, and free-form fields to directly set distance, speed, 67s, and rebirth count. This is a static site with no backend, so it's not real security — just enough to keep it out of random players' hands rather than anyone who happens to view the page source.
...
- **Leaderboard**: sign in with Google from the shop's LEADERBOARD tab to save your best distance, top speed, achievements, systems passed, Engine level and rebirths to a global leaderboard (Firebase Auth + Firestore). Sign-in uses Google Identity Services (Google's own official sign-in button, rendered directly on the page) rather than Firebase's popup or redirect helpers, both of which turned out to be unreliable on GitHub Pages — this exchanges a Google ID token for a Firebase credential without ever leaving the page. There's also a **temporary name-only option** right below it — just type a name, no real login — as a stopgap in case Google sign-in is still being sorted out; those entries are unverified (anyone can type any name) and marked with a "GUEST" tag on the leaderboard so they're never confused with a real Google-verified entry. Playing without signing in still works exactly the same — it's opt-in.
...
2. **Authentication → Sign-in method** → also enable the **Anonymous** provider, for the temporary name-only option (confirmed still needed: tested locally and got `auth/admin-restricted-operation`, which is exactly what Firebase returns when Anonymous auth isn't enabled).
3. Also under **Google's OAuth client** in [Google Cloud Console → Credentials](https://console.cloud.google.com/apis/credentials) (same project — this is separate from Firebase's own settings): add `https://thiessosterloh-collab.github.io` under **Authorized JavaScript origins** on the auto-created Web client.
4. **Firestore Database** → create a database if you haven't (Native mode, any region).
5. **Firestore Database → Rules** → paste this so players can only write their own score, but everyone can read the board:
   ```
   rules_version = '2';
...
Until those are done, the LEADERBOARD tab still works without errors — it just shows "No scores yet". Sign-in failures are no longer silent: a red toast banner appears at the top of the screen (visible even on the start screen) with the actual Firebase error code, and a green one confirms a successful sign-in with your name.
