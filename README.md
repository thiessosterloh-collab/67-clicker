...
- **Stamina** depletes with every tap and only recharges once you're back on the ground (altitude 0) — not passively over time. Run it out and you're "exhausted" — no more manual thrust, and gravity hits far harder — until it recovers to 25% (after landing). The **Stamina** upgrade raises your capacity (exponentially per level), which is what actually lets you climb higher before you're forced to drop and recover.
...
- Beyond our own Solar System's edge (the heliopause), the journey to the far edge of the galaxy passes through **~90 procedurally-generated solar systems**, numbered as levels ("Level 6: entering system TRAPPIST-4459", with real-astronomy-style catalog names), spaced by order of magnitude across the real ~160,000-light-year span it takes to actually leave the Milky Way. Each system's planets and star are met one at a time in sequence — planet, planet, star, then genuine empty void before the next system — each with its own tight visibility window, rather than the whole system appearing as one cluttered cluster.
...
- Every genuine A/D alternation earns **67s**, a persistent currency (saved across sessions), and moving forward also earns 67s passively over time. Both scale up **exponentially** the further out you are (based on orders of magnitude of real distance), so income keeps pace with the exponentially pricier upgrades. Spend 67s in the shop (`☰` menu button, or "UPGRADES" on the start screen) on four upgrades:
  - **Engine** — passive thrust that keeps accelerating you even when you're not tapping (and while the tab is closed). Runs on a 60-second fuel tank; land back on the ground to refuel (watch the ENGINE FUEL bar). This is the primary progression path — the only realistic way to approach and exceed light speed. (up to level 6000)
...
- **Leaderboard**: sign in with Google from the shop's LEADERBOARD tab to save your best distance, top speed, achievements, systems passed, Engine level and rebirths to a global leaderboard (Firebase Auth + Firestore). Sign-in uses a full-page redirect to Google (not a popup — popups are unreliable on GitHub Pages due to Cross-Origin-Opener-Policy), so the page will briefly navigate away and back. Playing without signing in still works exactly the same — it's opt-in.
