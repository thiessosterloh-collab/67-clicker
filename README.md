...
- Distance is tracked in real meters and speed in real m/s, with real Earth gravity (9.81 m/s²) pulling you back — **2200x** harder whenever your current thrust (just the Engine, if any — this doesn't depend on tapping) can't hold you up on its own, or once you're actually falling. On top of that, speed also decays **proportionally** at those times (losing at least 20% of itself per second, on top of the flat deceleration), since a fixed force is meaningless against relativistic speeds — cooling off from a burned-out Engine at a million+ m/s now actually cools off, not just a fixed-size nibble off an enormous number. Alternating `A`/`D` presses apply a thrust impulse and cost stamina; only genuine alternation (not repeats) counts, and consecutive alternations build a combo that increases thrust.
