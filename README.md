...
- Distance is tracked in real meters and speed in real m/s, with real Earth gravity (9.81 m/s²) pulling you back — **2200x** harder once you're actually falling or have run out of stamina with no engine to compensate, so an unassisted fall is effectively instant (you crash back to the ground within a frame or two, not a graceful glide down). Alternating `A`/`D` presses apply a thrust impulse and cost stamina; only genuine alternation (not repeats) counts, and consecutive alternations build a combo that increases thrust.
